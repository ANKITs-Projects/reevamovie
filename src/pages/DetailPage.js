...
function DetailPage() {
  ...
  // Destructure loading and error from hooks
  const { data, loading, error } = useFetchDetail(`${params?.explore}/${params?.id}`)
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-xl text-neutral-400">Loading details...</p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="text-center mt-20 px-4">
        <h2 className="text-2xl font-bold text-red-600">Oops! Could not load page.</h2>
        <p className="text-neutral-400 mt-2">{error}</p>
        <p className="mt-4 text-sm text-neutral-500">
          On laptops, <strong>Ad-blockers</strong> often block API calls. 
  ...
  return (
    <div>
      {/* Existing UI code starts here... */}
      <div className="w-full h-[280px] relative hidden lg:block">
        <div className="h-full w-full">
          <img src={imgUrl + data?.backdrop_path} alt="" className="h-full w-full object-cover" />
      ...
      <div className="container mx-auto px-3 py-16 lg:py-0 flex flex-col lg:flex-row gap-5 lg:gap-10">
        <div className="relative mx-auto w-fit lg:-mt-28 lg:mx-0">
          <img src={imgUrl + data?.poster_path} alt="" className="h-80 w-60 object-cover rounded" />
          <button 
        ...
        <div>
          ...
          <div className="flex items-center my-3 gap-3">
            ...
            <span>|</span>
            <p>Duration : {(Number(data?.runtime) / 60).toFixed(1)} hour</p>
          </div>
          <Devider />
          <div>
            ...
            <Devider />
            <div>
              <h2 className="text-lg lg:text-2xl font-bold">Star Cast :</h2>
